...
function populateContent(content, linkHTML, iframeDocument) {
  ...
  document.getElementById("sidebarContent").innerHTML =
    result + linkHTML + extra; // Update content display
}
...
async function getRankedStatsForPuuids(puuidArray) {
  try {
    const rankedStatsArray = await Promise.all(
      puuidArray.map(fetchRankedStats)
    );
...
async function getChampionSelectChatInfo() {
  try {
    ...
    return teamChatInfo
      ? teamChatInfo.find((item) => item.type === "championSelect")
      : null;
  } catch (error) {
  ...
  try {
    ...
    try {
      ...
      const response = await fetch(
        `https://plugins/${decodeURI(pluginName)}/config.json`
      );
    ...
    const participants = await create(
      "GET",
      "//riotclient/chat/v5/participants"
    );
    const lobby = participants.participants.filter((participant) =>
      participant.cid.includes("champ-select")
    );
    ...
    const displayData = lobby.map((player, index) =>
      formatPlayerData(player, ranks[index], matchData[index])
    );
    const displayData2 = lobby.map((player, index) =>
      formatPlayerData2(player, ranks[index], matchData[index])
    );
    ...
    const urlnames = lobby
      .map((player) =>
        encodeURIComponent(`${player.game_name}#${player.game_tag}`)
      )
      .join("%2C");
    const urlnames2 = lobby
      .map((player) =>
        encodeURIComponent(`${player.game_name}#${player.game_tag}`)
      )
      .join(",");
    ...
    window.open(
      `https://porofessor.gg/pregame/${region}/${urlnames2}`,
      "ventana1",
...
function formatPlayerData(player, rank, matchData) {
  ...
  const kdaRatios = calculateKDA(
    matchData.killList,
    matchData.assistsList,
    matchData.deathsList
  );
...
function calculateKDA(killsArray, assistsArray, deathsArray) {
  const totalKills = sumArrayElements(
    killsArray
      .map((kills) =>
        typeof kills === "string" ? kills.split(",").map(Number) : [kills]
      )
      .flat()
  );
  const totalAssists = sumArrayElements(
    assistsArray
      .map((assists) =>
        typeof assists === "string" ? assists.split(",").map(Number) : [assists]
      )
      .flat()
  );
  const totalDeaths = sumArrayElements(
    deathsArray
      .map((deaths) =>
        typeof deaths === "string" ? deaths.split(",").map(Number) : [deaths]
      )
      .flat()
  );
...
async function create(method, endpoint, action) {
  ...
  try {
    ...
    console.error(
      `Error in create function for ${method} ${endpoint}: ${error}`
    );
    return null;
  }
}
